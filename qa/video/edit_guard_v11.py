...
def wording(m,t):
 if t<.667:return '同じ場所から開始。ケースを取りに、東へ向かう。',['青▲がプレイヤー','A・B・Cは警備中','まずはAに注目']
 ...
 if t<5.22:return '姿を見失っても、最後に見た場所を調べる。',['通常AIにも記憶がある','A・Bは捜索を続ける','プレイヤーは東側へ']
 ...
 if m=='rules':
  ...
  return '通常AI：A・Bが西の音を調べ、Cは保管庫を守る。',['A：西へ援護に向かう','B：援護を待って調査','C：保管庫を警備']
 ...
 return 'Jevの返答で配置を見直す。Aは東、Bは西へ。',['東の目撃・銃声のあと','西でまた物音がした','→ Aを東の警戒に残す','→ Bが西を見張る']
...
def draw_pass(m,t,kind,elapsed,speed):
 ...
 if kind=='result':title='ケース回収前に、敵が侵入を止めた。';lines=['通常の耐久で収録','被弾してプレイヤーが倒れる','保管庫には到達できず']
 ...
 label='開始前・一時停止' if kind=='intro' else '結果・一時停止' if kind=='result' else f'{speed:g}倍速'
 ...
 if kind=='intro':
  ...
  if elapsed<1.8:
   im=Image.new('RGB',(W,H),BG);d=ImageDraw.Draw(im)
   txt(d,(65,27),'① 通常AI：この配置から開始' if m=='rules' else '② 同じAI ＋ Jev：同じ配置から開始',44)
   txt(d,(65,811),'開始前の静止画。右下に移ってから、動き始めます。',34)
...
# Both panels use exactly the same fixed world crop, enlarged 1.7x compared
# with v10's small overview panels (about 1.5x). No tracking camera hides separation.
ZOOM=(300,328,950,691)
def split(t,kind):
 im=Image.new('RGB',(W,H),BG);d=ImageDraw.Draw(im)
 heading='③ 同じ時刻で比較。Aの行き先に注目。'
 if t>=contact_time('jev'):heading='東に残ったAが、戻ってきたプレイヤーを発見。'
 txt(d,(35,22),heading,44)
 txt(d,(35,88),'同じ範囲を拡大 ／ 赤い線は、11秒以降にAが通った道',28)
 for m,x in [('rules',35),('jev',825)]:
  ...
  elif m=='rules':label='Aは西へ移動し、Bの調査を援護。'
  ...
  if t>=contact_time('jev'):detail='この時点では、まだ見つけていない。' if m=='rules' else '青▲が戻ってきたところを捉えた。'
...
def render(t):
 ...
 if kind=='divider':
  im=Image.new('RGB',(W,H),BG);d=ImageDraw.Draw(im)
  txt(d,(95,255),'次は、同じAIにJevを追加',56)
  txt(d,(100,380),'同じスタート地点から、同じ操作で。',44)
  txt(d,(100,491),'最後に、配置が分かれる場面を並べて見ます。',34)
...
if __name__=='__main__':
 (P/'guard-v11-chapters.json').write_text(json.dumps(segments,ensure_ascii=False,indent=2),encoding='utf-8')
 checks=[0,1.8,2.3,3.3,4.6]+[s['begin']+.5 for s in segments if s['kind']!='intro']
 for n,t in enumerate(checks):render(t)[0].save(P/f'guard-v11-check-{n:02d}.jpg',quality=94)
 split(14.5,'split_hold')[0].save(P/'guard-v11-poster.jpg',quality=95)
 if '--stills' in sys.argv:raise SystemExit(0)
 out=P/'RELAY-Jev-guard-v11.mp4'
 ...
 (P/'guard-v11-timeline.json').write_text(json.dumps(timeline,ensure_ascii=False,indent=2),encoding='utf-8')

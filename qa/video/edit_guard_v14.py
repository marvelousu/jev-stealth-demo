...
def draw_pass(m,t,kind,elapsed,speed):
 ...
 txt(d,(35,14),'① 通常AI' if m=='rules' else '② 通常AI ＋ Jev',28,GREEN)
 ...
 if kind=='intro':
  ...
  if elapsed<1.8:
   im=Image.new('RGB',(W,H),BG);d=ImageDraw.Draw(im)
   txt(d,(65,27),'① 通常AI' if m=='rules' else '② 通常AI ＋ Jev',44)
...
def split(t,kind):
 ...
 for m,x in [('rules',35),('jev',825)]:
  ...
  txt(d,(x,145),'通常AI' if m=='rules' else '通常AI ＋ Jev',40,GREEN)
...
if __name__=='__main__':
 (P/'guard-v14-chapters.json').write_text(json.dumps(segments,ensure_ascii=False,indent=2),encoding='utf-8')
 checks=[0,1.8,2.3,3.3,4.6]+[s['begin']+.5 for s in segments if s['kind']!='intro']
 for n,t in enumerate(checks):render(t)[0].save(P/f'guard-v14-check-{n:02d}.jpg',quality=94)
 split(14.5,'split_hold')[0].save(P/'guard-v14-poster.jpg',quality=95)
 if '--stills' in sys.argv:raise SystemExit(0)
 out=P/'RELAY-Jev-guard-v14.mp4'
 ...
 (P/'guard-v14-timeline.json').write_text(json.dumps(timeline,ensure_ascii=False,indent=2),encoding='utf-8')

...
def render(t):
 ...
 if kind=='divider':
  im=Image.new('RGB',(W,H),BG);d=ImageDraw.Draw(im)
  txt(d,(95,255),'次は同じAIにJevを追加',56)
  txt(d,(100,380),'同じスタート地点から同じ操作で。',44)
  txt(d,(100,491),'最後に配置が分かれる場面を並べて見ます。',44)
  meta=dict(mode='divider',simulation_time=None,caption='同じスタートからJev追加版')
 elif kind=='closing':
  closing=Image.new('RGB',(W,H),BG);d=ImageDraw.Draw(closing)
  sentence='同じAIでも作戦の選び方で守り方が変わる。'
  width=d.textbbox((0,0),sentence,font=fonts[44])[2]
  txt(d,((W-width)//2,405),sentence,44)
  if elapsed<.6:
   previous=split(15.5,'split_end')[0]
   im=Image.blend(previous,closing,elapsed/.6)
  else:im=closing
  meta=dict(mode='closing',simulation_time=None,caption=sentence)
...
if __name__=='__main__':
 (P/'guard-v12-chapters.json').write_text(json.dumps(segments,ensure_ascii=False,indent=2),encoding='utf-8')
 checks=[0,1.8,2.3,3.3,4.6]+[s['begin']+.5 for s in segments if s['kind']!='intro']
 for n,t in enumerate(checks):render(t)[0].save(P/f'guard-v12-check-{n:02d}.jpg',quality=94)
 split(14.5,'split_hold')[0].save(P/'guard-v12-poster.jpg',quality=95)
 if '--stills' in sys.argv:raise SystemExit(0)
 out=P/'RELAY-Jev-guard-v12.mp4'
 ...
 (P/'guard-v12-timeline.json').write_text(json.dumps(timeline,ensure_ascii=False,indent=2),encoding='utf-8')
